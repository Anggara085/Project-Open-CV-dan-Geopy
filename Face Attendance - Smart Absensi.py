...
# Menentukan lokasi kampus
campus_location = (-6.9686682, 110.4668120)  # Koordinat kampus Anda

def get_current_location():
    # Sebagai contoh, kita langsung mengembalikan koordinat tertentu.
    # Pastikan Anda mengimplementasikan logika yang sesuai untuk mendapatkan koordinat aktual.
    # Misalnya, dengan API atau perangkat GPS yang terhubung.
    # Misalnya menggunakan placeholder koordinat:
    return (-6.9686682, 110.4668120)  # Sesuaikan dengan koordinat lokasi Anda

def check_location(radius):
    current_location = get_current_location()
    print(f"Current Location: {current_location}")
    distance = geodesic(current_location, campus_location).meters
    print(f"Distance from campus: {distance} meters")
...
def trainingWajah():
    ...
    def getImageLabel(path):
        ...
        for imagePath in imagePaths:
            # Tambahkan pengecekan jenis file
            if os.path.isfile(imagePath) and imagePath.endswith(('.png', '.jpg', '.jpeg')):
    ...
    try:
        ...
        with open("Attendance.csv", 'r+') as f:
            ...
            if name not in namelist:
                ...
                f.writelines(f'\n{name},{yourclass},{yournim},{dtString},{current_location[0]},{current_location[1]}')
    except PermissionError as e:
        intructions.config(text=f"Permission denied: {e}")
    except Exception as e:
        intructions.config(text=f"Error: {e}")

def absensiWajah():
    if not check_location(100):  # Radius 100 meter
        intructions.config(text="Anda berada di luar radius kampus!")
        return
...
# judul
judul = tk.Label(root, text="Face Attendance - Smart Absensi", font=("Roboto", 34), bg="#20bebe", fg="white")
canvas.create_window(350, 80, window=judul)
#credit
made = tk.Label(root, text="Made by Anggara Putra Meldyantono", font=("Times New Roman", 13), bg="Green", fg="white")
...
label1 = tk.Label(root, text="Nama Siswa", font="Roboto", fg="white", bg="Green")
...
label2 = tk.Label(root, text="NIM", font="Roboto", fg="white", bg="Green")
...
label3 = tk.Label(root, text="Kelas", font="Roboto", fg="white", bg="Green")
canvas.create_window(65, 250, window=label3)

global intructions

# tombol untuk rekam data wajah
intructions = tk.Label(root, text="Welcome", font=("Roboto", 15), fg="white", bg="Green")
